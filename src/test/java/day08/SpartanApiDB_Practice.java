...
public class SpartanApiDB_Practice {
    ...
    public void testSearch() {
        ...
        int resultCount = response.path("totalElement");
    ...
    public void testSearchVerifyAllId() {
        ...
        List<String> IDListFromID=response.jsonPath().getList("content.id",String.class);
     ...
     //   System.out.println("resultCount = " + resultCount);
       ...
       assertEquals(IDListFromID.size(),idListFromDB.size());

        // how to assert 2 list have same content
        assertEquals(idListFromDB, IDListFromID);
